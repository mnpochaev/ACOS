...
int main(int argc, char *argv[])
{
    ...
    if (!sh) {
        fprintf(stderr, "gardener1: не удалось подключиться к shared memory\n");
        return 1;
    }
    ...
    printf("gardener1 (pid=%d) стартовал\n", getpid());
    ...
    run_gardener(1, sh, mutex, move_time_us, proc_time_us);

    printf("gardener1 (pid=%d) завершил работу\n", getpid());
